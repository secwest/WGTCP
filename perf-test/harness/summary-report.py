...
L.append("")
L.append("### Coverage completion")
L.append("")
L.append("The campaign initially left cells incomplete on `HIGH-x64`, `HIGH-arm`, and `MAX-x64` when separate `ssh` and `scp` collection attempts timed out. Three follow-up campaigns (`rg-wgtcpbase-p2p-gap2` for losses 0–5%, `rg-wgtcpbase-p2p-gap3` for losses 10/20%, and `rg-wgtcpbase-p2p-gap4` for `HIGH-arm` 10/20%) made collection resilient with one `ssh + tar + base64` stream per cell and retry-with-backoff. The 0–5% pass completed all four affected pairs at 100% (288/288, zero retries), and the 10/20% pass completed `HIGH-x64` and `MAX-x64`. For the six remaining `HIGH-arm` cells, a 360-second wall-clock bound kept the high-loss `short-transfer` workload deterministic; all 32/32 `HIGH-arm` TCP-WG cells then captured cleanly with no retries. Later transport work additionally added explicit writer wakeups, bounded transmit queues, and short-write, queue-failure, fatal-I/O, and carrier-replacement recovery.")
L.append("")
